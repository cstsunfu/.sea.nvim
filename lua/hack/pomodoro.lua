...
local menu = function(tab)
    ...
    for idx, proj in ipairs(tab) do
        ...
        table.insert(option, idx, {
            ...
            key = tostring(idx),
            action = function()
                M.cur_proj = vim.fn.input(
                    'Confirm Project Name, "x" means "Others": ',
                    proj,
                    "custom,v:lua.pomodoro.proj_complete"
                )
            end,
        })
    end
    ...
    table.insert(option, #option + 1, {
        label = "Current project directory name",
        key = "c",
        action = function()
    ...
    table.insert(option, #option + 1, {
        ...
        action = function()
            local proj =
                vim.fn.input('Enter Project Name, "x" means "Others": ', "", "custom,v:lua.pomodoro.proj_complete")
            if proj == "x" then
                proj = "Others"
            end
            M.cur_proj = proj
    ...
    util.menu("Press Key For Select Project Name", option, "Press key")
end
...
M.start_pomodoro = function()
    if M.reserve ~= nil then
        vim.notify(
            'There is Already a Pomodoro Clock Running on Project "'
                .. (M.cur_proj or "NO NAME")
                .. '".\nYou Should Stop/Cancel it.',
